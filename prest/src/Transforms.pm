...
# Arguments: pending DOM, top-level DOM, details hash reference
sub Filter{
    my ($dom, $topdom, $details) = @_;
...
sub DocInfo {
	...
	foreach $field (@content) {
		...
		if ($element_seen{$name} > 1 && $name =~ /abstract/) {
		    ...
		    my $topic = new DOM('topic', classes=>[ $name ]);
...
sub create_title {
	...
	if (! $prev_title) {
	    $dom->{attr}{title} =
		  $ttext; #RST::NormalizeName($ttext, 'keepcase');
	...
	if ($main::opt_D{keep_title_section} && ! defined $prev_title) {
		...
		$subtitle->{attr}{ids} = [ RST::NormalizeId($ttext) ];
		$subtitle->{attr}{names} = [ RST::NormalizeName($ttext) ];
...
sub Class {
    ...
    return RST::system_message(2, $dom->{source}, $dom->{lineno},
...
sub Contents {
	...
	(sub {
		 ...
		 if (! defined $details->{depth} ||
			 ...
			 $li->append($para);
			 my $ref = new DOM('reference', ids=> [ $id ],
	...
	$toc->Recurse
	    (sub {
		 ...
		 if ($dom->{tag} eq 'bullet_list') {
			...
			&& (! defined $details->{depth} ||
			    @list <= $details->{depth})) {
		     my $gen = new DOM('generated', classes=>['sectnum']);
		     $gen->append(newPCDATA DOM($prefix . join('.',@list)
						. $suffix . ("\xa0"x3)));
		     $dom->prepend($gen);
		 }
		 return 0;
...
sub AutoFootnotes {
	...
	(sub {
		 ...
		 if ($dom->{attr}{auto}) {
		     ...
		     $RST::REFERENCE_DOM{$tag}{$dom->{attr}{names}[0]} = $dom
			 if defined $dom->{attr}{names};
		     $RST::REFERENCE_DOM{$tag}{$dom->{attr}{ids}[0]} = $dom;
		 }
	     }
	     return 0;
	 }
	...
	(sub {
		 ...
		 while ($next->{tag} eq 'target' &&
			...
			(defined $next->{attr}{refname} ||
			 ...
			 {
			     push @chain, $next;
			     my @targets = @{$RST::ALL_TARGET_NAMES{$name}}
			     if defined $RST::ALL_TARGET_NAMES{$name};
...
sub CitationReferences {
	...
	if ($tag =~ /^(?:(citation|substitution)_reference)$/) { 
	    ...
	    $target = ($RST::REFERENCE_DOM{"$what.lc"}{lc $name})
...
sub References {
	...
	(sub {
		 ...
		 while (defined $target) {
			 ...
			 my $refid =
			     RST::NormalizeId($dom->{attr}{refname});
			 $dom->{attr}{refid} =
...
sub TargetNotes {
	...
	my $dom = new DOM('footnote', auto=>1, ids=>[ $id ],
...
sub Decorations {
	...
	$para->append(newPCDATA DOM("Generated by $main::TOOL_ID from "),
		      $ref,
...
sub Messages {
   ...
   if (@SYSTEM_MESSAGES > 0) {
       my $errsec = new DOM('section', classes=>['system-messages']);
       $dom->append($errsec);
       my $title = new DOM('title');
       $errsec->append($title);
