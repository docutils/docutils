...
*.doctest
    Files that contain "semiliterate testing" [1]_ - that is, descriptive text
    (written with reST) surrounding doctest blocks. The idea is that these
    files should be passed to (for instance) pydps.py (or whatever it is
    called at the moment) to be tested by Tim Peters' doctest - for instance::
