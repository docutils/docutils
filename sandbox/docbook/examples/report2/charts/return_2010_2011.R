...
x = rates$week
title='Return Rates, 2010, 2011'
y1 = rates$rate.returns.2010
y2 = rates$rate.returns.2011
png(file="return_rate_2010_2011.png", width=5, height=3.5, units="in", res=res)
xlab = "Week in Year"
xlab.text = xlab
ylab = "units/man hour" 
...
error_bars = predict(lm(y1[20:40]~x[20:40]), interval='confidence')
#segments(20,error_bars[1], 40, error_bars[20], col=line_colors[1])
error_bars = predict(lm(y2[20:40]~x[20:40]), interval='confidence')
#segments(20,error_bars[1], 40, error_bars[20], col=line_colors[2])
