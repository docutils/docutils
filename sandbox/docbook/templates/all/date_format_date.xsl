...
<xsl:template name="date:_format-date">
   ...
   <xsl:choose>
      ...
      <xsl:when test="not(contains('GyMdhHmsSEDFwWakKzbB', $char))">
         <xsl:message>
         ...
         <xsl:choose>
             <xsl:when test="$char = 'b'">
               <xsl:choose>
                  ...
                  <xsl:otherwise>
                      <xsl:choose>
                          <xsl:when test="string($month) = 1">
                              <xsl:text>Jan</xsl:text>
                          </xsl:when>
                          <xsl:when test="string($month) = 2">
                              <xsl:text>Feb</xsl:text>
                          </xsl:when>
                          <xsl:when test="string($month) = 3">
                              <xsl:text>Mar</xsl:text>
                          </xsl:when>
                          <xsl:when test="string($month) = 4">
                              <xsl:text>Apr</xsl:text>
                          </xsl:when>
                          <xsl:when test="string($month) = 5">
                              <xsl:text>May</xsl:text>
                          </xsl:when>
                          <xsl:when test="string($month) = 6">
                              <xsl:text>Jun</xsl:text>
                          </xsl:when>
                          <xsl:when test="string($month) = 7">
                              <xsl:text>Jul</xsl:text>
                          </xsl:when>
                          <xsl:when test="string($month) = 8">
                              <xsl:text>Aug</xsl:text>
                          </xsl:when>
                          <xsl:when test="string($month) = 9">
                              <xsl:text>Sep</xsl:text>
                          </xsl:when>
                          <xsl:when test="string($month) = 10">
                              <xsl:text>Oct</xsl:text>
                          </xsl:when>
                          <xsl:when test="string($month) = 11">
                              <xsl:text>Nov</xsl:text>
                          </xsl:when>
                          <xsl:when test="string($month) = 12">
