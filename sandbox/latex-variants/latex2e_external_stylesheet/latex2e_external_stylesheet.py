...
class Writer(writers.Writer):
    ...
    settings_spec = (
        ...
        (('Specify documentclass.  Default is "article".',
         ...
         ('Location of style file, used verbatim. '
          'Default is ("docutils-latex2e"). '
          'Overrides --stylesheet-path.',
          ['--stylesheet'],
          {'default': '', 'metavar': '<file>',
           'overrides': 'stylesheet_path'}),
         ('Location of style file relative to the current working '
          'directory.  Overrides --stylesheet.',
          ['--stylesheet-path'],
          {'metavar': '<file>', 'overrides': 'stylesheet'}),
         ('Embed the style in the output LaTeX file.  The style '
          'file must be accessible during rst->LaTeX conversion.',
          ['--embed-stylesheet'],
          {'default': 0, 'action': 'store_true',
           'validator': frontend.validate_boolean}),
         ('Link to the style in the output LaTeX file. The file is referenced '
          'in the LaTeX header with \\usepackage{} and input by `latex`. '
          'File must be accessible during LaTeX->PDF (or DVI) conversion.'
          'This is the default.',
          ['--link-stylesheet'],
...
class LaTeXTranslator(nodes.NodeVisitor):
    ...
    def __init__(self, document):
        ...
        self.head_prefix.append('% Docutils LaTeX Style\n')
        stylesheet = utils.get_stylesheet_reference(self.settings)
        if not stylesheet:
            stylesheet = 'docutils-latex2e'
        if settings.embed_stylesheet:
            # relate stylesheet path to current working dir if set with
            # --stylesheet-path
            stylesheet = utils.get_stylesheet_reference(
                settings, os.path.join(os.getcwd(), 'dummy'))
            # resolve symbolic links and references (like '..' for parent dir)
            stylesheet = os.path.realpath(stylesheet)
            # add default extension if none present
            if os.path.splitext(stylesheet)[1] == '':
                stylesheet += '.sty'
        ...
        self.head_prefix.extend(self.stylesheet)
