...
# this is a modification of the latex2e writer by Engelbert Gruber
# version ``__init__.py 5525 2008-03-02 02:35:31Z grubert``
# by G Milde 
    ...
    settings_spec = (
        ...
        (('Specify documentclass.  Default is "article".',
         ...
         ('When possibile, use the specified environment for literal-blocks. '
          'Default is quoting of whitespace special chars.',
          ['--literal-env'],
          {'choices': ['verbatim', 'Verbatim', 'verbatimtab', 
                       'listing', 'lstlisting'], 
           'default': ''}),
         ('When possibile, use verbatim for literal-blocks. '
          'Backwards compatibility alias for "--literal-block-env=verbatim"',
...
class LaTeXTranslator(nodes.NodeVisitor):
    ...
    # packages providing environments to typeset literal blocks
    literal_package = {"": "",
                       "verbatim": "",
    ...
    def __init__(self, document):
        ...
        self.head_prefix = [
              ...
              # package providing the literal block environment
              self.literal_package[self.settings.literal_env],
    ...
    def visit_literal_block(self, node):
        ...
        # the others we should  use the `alltt` environment.
        #
        ...
        if not self.active_table.is_open():
            ...
            self.body.append('\n')
            self.context.append('\n')
        if (self.settings.literal_env != '' 
            and (len(node) == 1) and isinstance(node[0], nodes.Text)
            # in case a parsed-literal containing just a "**bold**" word:
           ):
            self.verbatim = 1
            self.body.append('\n\\begin{%s}\n' % self.settings.literal_env)
            self.context.append('\n\\end{%s}\n' % self.settings.literal_env)
        else:
    ...
    def depart_literal_block(self, node):
        ...
        self.insert_none_breaking_blanks = 0
        self.literal_block = 0
        self.body.append(self.context.pop())  # end verbatim env
        self.body.append(self.context.pop())  # end quote env (dummy in table)
