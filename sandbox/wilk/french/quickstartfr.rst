...
bonne apellation. Nous devrions plutôt le nommer "Relaxed Text" qui contient
quelques shémas logiques. Ces shémas sont interprétés par un convertisseur
HTML pour produire "Very Structured Text" (un texte très structuré) qui pourra
être utilisé par un navigateur web.

Le shéma le plus simple est le **paragraphe** (quickref__).
...
est assez malin.
Par example, cet * asterisque est traité correctement. Si vous
souhaitez par contre \*entourer un texte par des astérisques* 
**sans** qu'il soit en italique, il est nécessaire d'indiquer que
l'astérique ne doit pas être interprété. Pour cela il suffit de placer
un anti-slash juste avant lui, comme ça "``\*``" (quickref__), ou
en l'entourant de deux guillemets (litteral), comme cela ::
...
de **définitions**. Dans chaque cas, nous pouvont avoir autant
...
__ quickref.html#enumerated-lists
  ...
  -- comme vous préférez. Toutes ces formes sont reconnues::
    ...
    A. Lettres en majuscule
       qui continue sur plusieur ligne

       avec deux paragraphe et tout !
...
terminé lorsqu'une ligne retombe au niveau du décalage précédent. Par exemple::
  ...
  Un exemple::
   ...
   Regardez ici, je suis déscendu d'un niveau.
   (mais pas assez)
...
Notez que si le pragraphe contient seulement "``::``", il est ignoré.
...
**entêtes de section**. C'est à dire une seule ligne de texte (d'un
ou plusieurs mots) avec un ornement : juste en dessous et éventuellement
dessus aussi, avec des tiréts "``-----``", equals "``=====``", tildes
...
Notez que les entêtes de section sont utilisable comme cible de liens,
...
d'utiliser les guillemets "```Styles de texte`_``".
...
égal, mais sont différents et sans relation. Le texte et l'ornement doit
être de la même taille pour l'esthétisme.
...
Cette introduction montre les possibilités courrantes de reStructuredText,
mais il y en a bien d'autres à explorer. Le manuel de référence utilisateur
'Quick reStructuredText`_ est conseillé maintenant. Pour les détails complets
...
Le `Docutils project web site`_ comporte d'avantages d'informations.
